...
export default function HotelBudgetImpact({
  open,
  currentHotel,
  selectedHotel,
  currency,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-[#fb7185]/25 bg-[#0D1324]/95 p-5 shadow-[0_30px_70px_rgba(0,0,0,0.5)] backdrop-blur-2xl sm:p-6">
        <button
          ...
          className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-lg text-[#948e9c] transition hover:bg-white/[0.05] hover:text-white"
        >
          <X size={15} />
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-[#948e9c]">
              Review before changing accommodation.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 space-y-3 rounded-xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <BudgetRow
            label="New Hotel"
            value={`${currency} ${selectedHotelTotal.toLocaleString()}`}
            accent="amber"
          />
